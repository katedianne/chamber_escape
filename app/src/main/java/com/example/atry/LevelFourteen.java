...
public class LevelFourteen extends AppCompatActivity implements View.OnTouchListener {

    Application application = com.example.atry.Application.getOurIntance();
    private long currentTime = 0, timeInvincible = 0;
    private ImageView wiz, plat1, plat2, obs1, obs2, obs3, obs4, goal;
...
//                Toast.makeText(getApplicationContext(), "DEAD", Toast.LENGTH_SHORT).show();
                    ...
                    if (!application.isRevived()) {
                        Intent intent = new Intent(LevelFourteen.this, MiniGame.class);
                        startActivity(intent);
                    } else {
                        Intent intent = new Intent(LevelFourteen.this, GameOver.class);
                        startActivity(intent);
                    }
                    gameOver = true;
                }
            ...
            if (Collision.checkColl(goal, wiz)) {
                Intent intent = new Intent(LevelFourteen.this, Escaped.class);
                startActivity(intent);
                gameOver = true;
            }
        }
