...
public class LevelThirteen extends AppCompatActivity implements View.OnTouchListener {

    Application application = com.example.atry.Application.getOurIntance();
    private long currentTime = 0, timeInvincible = 0;
    private ImageView wiz, plat1, obs1, obs2, obs3, obs4, goal;
...
//            wizard.setImageDrawable(wizardRightRun);
        ...
        if (action_down || action_up || action_left || action_right) {
            if(currentTime > timeInvincible) {
                if (Collision.checkColl(obs1, wiz)|| Collision.checkColl(obs2, wiz)|| Collision.checkColl(obs3, wiz)|| Collision.checkColl(obs4, wiz)) {
//                Toast.makeText(getApplicationContext(), "DEAD", Toast.LENGTH_SHORT).show();
                    ...
                    if (!application.isRevived()) {
                        Intent intent = new Intent(LevelThirteen.this, MiniGame.class);
                        startActivity(intent);
                    } else {
                        Intent intent = new Intent(LevelThirteen.this, GameOver.class);
                        startActivity(intent);
                    }
                    gameOver = true;
                }
            ...
            if (Collision.checkColl(goal, wiz)) {
                Intent intent = new Intent(LevelThirteen.this, Escaped.class);
                startActivity(intent);
                gameOver = true;
            }
        }
        ...
        if (wizardY > plat1.getY() - wiz.getHeight() && wizardX + wiz.getWidth() > plat1.getX()) wizardY = plat1.getY() - wiz.getHeight();
